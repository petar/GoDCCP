...
func (c *Conn) step8_OptionsAndMarkAckbl(h *Header) error {
	...
	if err := c.scc.OnRead(&FeedbackHeader{
		...
		c.logger.E("Error", "Sender CC unknown read error", h)
	}
	...
	if err := c.rcc.OnRead(&FeedforwardHeader{
		...
		c.logger.E("Error", "Receiver CC unknown read error", h)
	}
	return nil
}
...
func (c *Conn) step11_ProcessRESPOND(h *Header) error {
	...
	if h.Type == Request {
		...
		if h.Type != Ack && h.Type != DataAck {
			...
			c.logger.E("Event", "Entering OPEN on non-Ack packet", h)
		}
		c.gotoOPEN(h.SeqNo)
	}
	return nil
