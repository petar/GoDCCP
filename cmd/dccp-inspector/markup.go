...
const (
	...
	css = 
		...
		`td.time-abs { background: #fafafa; width: 100px; text-align: left }` +
		...
		`td.mark-0.time, td.mark-0.time-abs { background: ` + optMark0Color + `; color: white; }` +
		`td.mark-1 { border-top: 3px solid ` + optMark1Color + ` }` +
		`td.mark-1.time, td.mark-1.time-abs { background: ` + optMark1Color + `; color: white; }` +
		`td.mark-2 { border-top: 3px solid `+ optMark2Color + ` }` +
		`td.mark-2.time, td.mark-2.time-abs { background: ` + optMark2Color + `; color: white; }` +
		// Tooltips
			...
			`{{ $ev := .Event }}{{ $seqno := .SeqNo }}{{ $ackno := .AckNo }}{{ $trace := .Trace }}` +
				...
				`{{ with .Server }}` +
					...
					`<td class="server state ev_{{ $ev }} {{ $ne }}" seqno="{{ $seqno }}" ackno="{{ $ackno }}">` +
						`<pre>{{ .State }}</pre></td>` + 
				`{{ end }}` +
				`<td class="time-abs ev_{{ $ev }}" seqno="{{ $seqno }}" ackno="{{ $ackno }}"><pre>{{ .TimeAbs }}</pre></td>` +
			`</tr>` +
		`{{ end }}`
)
